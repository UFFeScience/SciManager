...
var validator = (function($) {
	...
	*/
	message = {
		...
		empty			: 'Campo não pode ser vazio',
		min				: 'Nome muito curto',
		max				: 'Nome muito longo',
		number_min		: 'Valor muito baixo',
		number_max		: 'Valor muito alto',
		url				: 'Url inválida',
		...
		complete		: 'Formulário incompleto',
	...
	*/
	tests = {
		...
		number : function(a) {
			...
			// check if there is max length & field length is greater than the allowed
			else if (lengthRange && lengthRange[1] && a.length > lengthRange[1] && a > 0) {
				alertTxt = message.max;
				return false;
			}
	...
	*/
    mark = function(field, text) {
		...
		if (item.hasClass('bad')) {
        	...
        	if (!item.find('.alert').length && item.find('.alert').text() != text) {
	...
	*/
	...
	function prepareFieldData(el) {
		// get chosen hidden select
		if ($(el).hasClass('chosen-component-input')) {
			field = $(el).find('select:required');
		}
		else {
			field = $(el);
		}
	...
	*/
	function checkField() {
		...
		if (this.type !='hidden' && $(this).is(':hidden:not(select)')) {
			return true;
		// validate chosen select
		}
		else if ($(this).hasClass('chosen-component-input') && $(this).find('select') && !$(this).find('select').prop('required')) {
			return true;
		}

		prepareFieldData(this);

		if ($(this).hasClass('chosen-component-input')) {
			setTimeout(function() {
				return doValidation();
		    }, 100);
		}
